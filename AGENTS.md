...
## RiservApp development rules — condivise da Claude e Codex

Prima di pianificare, implementare o revisionare: leggere questo file, poi
TASKS.md e il documento del lavoro indicato lì. Queste regole valgono per
entrambi gli agenti. AGENTS.md è la fonte unica; CLAUDE.md lo importa.
...
- Non aggiungere dipendenze senza autorizzazione di Michele.
- Prima di modificare struttura o persistenza del database, spiegare la migration:
  dati coinvolti, compatibilità, conservazione dei dati e ripristino.
- Ogni feature deve compilare.
- Nessuna funzione duplicata: cercare e riusare la logica esistente.
- Non eliminare funzionalità esistenti per risolvere un bug.

### Testing
Prima di concludere:
1. Eseguire la build dei componenti interessati.
2. Eseguire i test pertinenti, inclusi quelli di regressione disponibili.
3. Controllare e riportare i warning significativi.

Usare gli script e la configurazione presenti nel repository. Riportare comandi,
esiti e verifiche non eseguite con il motivo; un controllo bloccato non è passato.
Per modifiche esclusivamente documentali verificare diff, riferimenti e coerenza;
build e test applicativi non sono necessari.
...
problemi, dirlo senza presentare la review come garanzia di assenza di bug.

### Coordinamento e Git
- Michele decide obiettivi e priorità.
- ChatGPT Work: architettura, piano e criteri di accettazione.
- Claude Code: implementazione e verifiche.
- Codex CLI: revisione indipendente sul diff concordato.
- Il flusso è: Michele → piano → Claude implementa → Codex controlla →
  correzioni e nuova verifica → Git. È un accordo di lavoro, non un collegamento
  automatico fra gli strumenti.
- Prima di lavorare controllare branch, stato Git e modifiche già presenti.
  Preservare il lavoro altrui; aggiungere al commit solo file del proprio compito.
- Un solo agente scrive nello stesso albero alla volta; il reviewer legge.
  Per scritture contemporanee usare worktree distinti.
- Per la review annotare commit di partenza e commit finale, oppure specificare
  esplicitamente il diff locale da controllare.
- main resta la fonte di verità. Al termine di una sessione di modifica aggiornare
...
richiesto prepara obiettivo, ambito, criteri di accettazione, rischi e passi di
implementazione. Spiega eventuali migration prima delle modifiche. Se non hai
accesso ai file, richiedili senza presumere che siano stati caricati.

### Prompt per lo sviluppatore
Leggi AGENTS.md e TASKS.md. Implementa il lavoro concordato mantenendo l'ambito.
Indica la base Git, esegui build e test pertinenti, controlla i warning e consegna
al reviewer diff o commit, risultati delle verifiche e limiti ancora aperti.

### Prompt per il reviewer
Leggi AGENTS.md e TASKS.md. Revisiona il diff indicato rispetto alla base concordata
senza modificare i file. Applica la checklist Review e verifica le prove riportate
dallo sviluppatore. Restituisci problemi con gravità, file/riga e scenario concreto,
poi verifiche eseguite e rischi residui. Se non trovi problemi, dichiaralo.
