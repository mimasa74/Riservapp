...
export const BachecaScreen = ({ posts, hunterName, onAddPost, onDeletePost, onMarkRead }: BachecaScreenProps) => {
  ...
  // Auto-segna come letti tutti i post visibili all'apertura
  React.useEffect(() => {
    if (!hunterName || posts.length === 0) return;
    const storageKey = `riservapp_letti_${hunterName}`;
    const già: string[] = JSON.parse(localStorage.getItem(storageKey) || '[]');
    const daLeggere = posts.filter(p => !già.includes(p.id)).map(p => p.id);
    if (daLeggere.length === 0) return;
    onMarkRead(daLeggere);
    localStorage.setItem(storageKey, JSON.stringify([...già, ...daLeggere]));
  }, [posts, hunterName]);
  ...
  return (
    ...
    {showAdminModal && (
      ...
      {/* Header Bacheca */}
      <div style={{ background: '#ECEDE1', display: 'flex', alignItems: 'center', gap: 16, padding: '16px 18px' }}>
        <div
          style={{ width: 80, height: 80, flexShrink: 0, touchAction: 'none', position: 'relative' }}
        ...
        <div style={{ flex: 1, paddingLeft: 30 }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: '#5C6B3A', textTransform: 'uppercase', letterSpacing: '0.09em', fontFamily: '-apple-system, sans-serif', lineHeight: 1.4 }}>
            Riserva Val di Tovel
          </p>
          <p style={{ fontSize: 30, fontWeight: 800, color: '#1A1A14', textTransform: 'uppercase', lineHeight: 1.05, letterSpacing: '-0.02em' }}>
            Bacheca
          </p>
          {isAdmin && (
      ...
      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {/* Pulsante + (solo admin, quando form chiuso) */}
        {isAdmin && !showForm && (
          <button
        ...
        {posts.map(post => (
          <PostCard key={post.id} post={post} isAdmin={isAdmin} onDelete={onDeletePost} />
        ))}
